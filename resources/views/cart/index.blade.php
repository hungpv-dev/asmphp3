                                ...
                                <div class="cart__table">
                                    <table class="cart__table--inner">
                                        ...
                                        <tr class="cart__table--header__items">
                                            ...
                                            <th class="cart__table--header__list">Giá tiền</th>
                                                ...
                                                <tr class="cart__table--body__items">
                                                    <td class="cart__table--body__list">
                                                        <div class="cart__product d-flex align-items-center">
                                                            <button class="cart__remove--btn deleteCart" data-id="{{$cart->id}}" aria-label="search button" type="button">
                                                            ...
                                                            <div class="cart__content">
                                                                <h4 class="cart__content--title"><a href="{{route('product.detail',[$cart->conditions['slug'],$cart->conditions['product_id']])}}">{{$cart->name}}</a>
                                                                    @if($cart->conditions['price_seal'] > 0)
                                                                        <span class="text-danger fs-4 ms-2">(giảm {{$cart->conditions['price_seal']}}%)</span>
                                                                    @endif
                                                                </h4>
                                                                <span class="cart__content--variant">MÀU SẮC: {{$cart->attributes['color']}}</span>
                                                                <span class="cart__content--variant">KÍCH CỠ: {{$cart->attributes['size']}}</span>
                                                            </div>
                                                        </div>
                                                    </td>
                                                    <td class="cart__table--body__list">
                                                        @if($cart->conditions['price_seal'] > 0)
                                                            <span class="cart__price price_product"><del>{{number_format($cart->price)}}</del><br><span class="text-danger">{{number_format($cart->price - ($cart->conditions['price_seal'] / 100 * $cart->price))}}</span></span>
                                                        @else
                                                            <span class="cart__price price_product">{{number_format($cart->price)}}<span
                                                        @endif
                                                    </td>
                                                    <td class="cart__table--body__list">
                                                        <div class="quantity__box">
                                                            <button type="button" class="quantity__value quickview__value--quantity cartPage decrease" aria-label="quantity value" value="Decrease Value">-</button>
                                                            <label>
                                                                <input type="number" readonly data-id="{{$cart->id}}" class="quantity__number quickview__value--number count" min="1" value="{{$cart->quantity}}" data-counter/>
                                                            </label>
                                                            <button type="button" class="quantity__value quickview__value--quantity cartPage increase" aria-label="quantity value" value="Increase Value">+</button>
                                                        </div>
                                                    </td>
                                                    <td class="cart__table--body__list">
                                                        <span class="cart__price price_product end">{{number_format(($cart->price - ($cart->conditions['price_seal'] / 100 * $cart->price))*$cart->quantity)}}</span>
                                                    </td>
                                                </tr>
                                            @endforeach
                                        </tbody>
                                    ...
                                    <div class="continue__shopping d-flex justify-content-between">
                                        <a class="continue__shopping--link" href="{{route('product.show')}}">Tiếp tục mua sắm</a>
                                        <a class="continue__shopping--clear" href="{{route('cart.clear')}}" onclick="return confirm('Xóa hết sản phẩm trong giỏ hàng?')" >Giọn dẹp giỏ hàng</a>
                                    </div>
                                </div>
                            </div>
                            <div class="col-lg-4">
                                    ...
                                    <div class="cart__summary--total mb-20">
                                        <table class="cart__summary--total__table">
                                            ...
                                            <tr class="cart__summary--total__list">
                                                <td class="cart__summary--total__title text-left">Tiền hàng</td>
                                                <td class="cart__summary--amount text-right"><span id="price-product"></span> <span class="text-danger">VNĐ<span></td>
                                            </tr>
                                            <tr class="cart__summary--total__list">
                                                <td class="cart__summary--total__title text-left">Tổng tiền</td>
                                                <td class="cart__summary--amount text-right"><span id="price-total-product"></span> <span class="text-danger">VNĐ<span></td>
                                            </tr>
                                            </tbody>
                                        </table>
                                    </div>
                                    <div class="cart__summary--footer">
                                        <p class="cart__summary--footer__desc">Vận chuyển & thuế được tính khi thanh toán</p>
                                        <ul class="d-flex justify-content-between">
                                            <li><button class="cart__summary--footer__btn primary__btn cart" type="submit">Cập nhật giỏ hàng</button></li>
                                            <li><a class="cart__summary--footer__btn primary__btn checkout" href="{{route('order.info')}}">Thanh toán</a></li>
                                        </ul>
                                    </div>
                                </div>
                            </div>
...
@section('script')
    <script>
        $(document).on('click','.quickview__value--quantity.cartPage.increase',function(){
            let input = $(this).closest('.quantity__box').find('.quickview__value--number.count');
            let id = input.data('id');
            let value = input.val();
            updateCountCartPage(id,value,'up',input);
        });
        $(document).on('click','.quickview__value--quantity.cartPage.decrease',function(){
            let input = $(this).closest('.quantity__box').find('.quickview__value--number.count');
            let id = input.data('id');
            let value = input.val();
            updateCountCartPage(id,value,'down',input);
        });
        $(document).on('click','.deleteCart',function(){
            ...
            swal({
                ...
                .then((willDelete) => {
                    if (willDelete) {
                        removeCartIdPage(id,$(this));
                    }
                });
        });
        function updateCountCartPage(id, value, phepTinh,input){
        ...
        function removeCartIdPage(id,input){
            $.ajax({
                ...
                success: function (response){
                    ...
                    input.closest('.cart__table--body__items').remove();
                    totalPriceCart();
                },
                error: function(log){
                    console.log(log);
        ...
        function totalPriceCart(){
            let spans = $('.price_product.cart__price.end');
            let sum = 0;
            for(const span of spans){
                sum += +(span.innerText.replace(/,/g, ''));
            }
            let value = Math.floor(sum).toLocaleString('en-US');
            $("#price-product").text(value);
            $("#price-total-product").text(value)
        }
    </script>
@endsection
